"""Replay motion for G1-23dof robot.

Usage:
    # Bata Dias motion
    python scripts/mimic/replay_npz_23dof.py \
...
parser = argparse.ArgumentParser(description="Replay converted motions for G1-23dof.")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    print(f"Total frames: {motion.time_step_total}")
    print(f"Duration: {motion.time_step_total * sim_dt / args_cli.speed:.2f} seconds")
    print(f"Joints: {motion.joint_pos.shape[-1]}")
    print(f"Speed: {args_cli.speed}x")
    print(f"\nPlaying... (Close window to exit)\n")

    time_steps = torch.zeros(scene.num_envs, dtype=torch.long, device=sim.device)
    frame_skip = max(1, int(args_cli.speed))

    # Simulation loop
    while simulation_app.is_running():
        time_steps += frame_skip
