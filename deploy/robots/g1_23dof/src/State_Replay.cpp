...
// G1-23dof: 23 joints (no waist roll/pitch, no wrist pitch/yaw)
// Isaac Lab joint index -> SDK motor index mapping
    ...
    : dt_(1.0f / fps)
{
    ...
    while (std::getline(file, line)) {
        ...
        // CSV format for 23dof: root_pos(3) + root_quat(4) + joint_pos(23) = 30 columns
        // We only need joint positions (columns 7-29)
        if (row.size() >= 30) {
            Eigen::VectorXf joints(23);
            for (int i = 0; i < 23; ++i) {
                joints[i] = row[7 + i];
            }
    ...
    : FSMState(state_mode, state_string)
{
    ...
    // PD gains in SDK order (27 elements, zeros for non-existent joints)
    if (cfg["kp"]) {
        kp_ = cfg["kp"].as<std::vector<float>>();
    } else {
        kp_ = std::vector<float>(27, 50.0f); // default kp
    }

    if (cfg["kd"]) {
        kd_ = cfg["kd"].as<std::vector<float>>();
    } else {
        kd_ = std::vector<float>(27, 3.0f); // default kd
    }
...
void State_Replay::enter()
{
    ...
    // Set PD gains directly in SDK order (27 elements)
    // kp_/kd_ are in SDK order with zeros for non-existent joints (13,14,20,21)
    for (int i = 0; i < 27; ++i) {
    ...
    // Capture initial pose for blending (in Isaac Lab order)
    initial_pose_.resize(23);
    for (int i = 0; i < 23; ++i) {
        int sdk_idx = JOINT_IDS_MAP[i];
        initial_pose_[i] = lowstate->msg_.motor_state()[sdk_idx].q();
    }
    ...
    replay_thread = std::thread([this]() {
        ...
        while (replay_thread_running.load()) {
            ...
            if (elapsed < blend_duration_) {
                ...
                // Apply using joint_ids_map (Isaac Lab -> SDK)
                for (int i = 0; i < 23; ++i) {
                    int sdk_idx = JOINT_IDS_MAP[i];
                    lowcmd->msg_.motor_cmd()[sdk_idx].q() = blended[i];
                }
            } else {
                ...
                for (int i = 0; i < 23; ++i) {
                    int sdk_idx = JOINT_IDS_MAP[i];
                    lowcmd->msg_.motor_cmd()[sdk_idx].q() = target[i];
                }
            }
